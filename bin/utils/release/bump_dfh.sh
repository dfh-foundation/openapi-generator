...
declare -r dfh_version_regex="([0-9]+).([0-9]+).([0-9]+)-DFH-([0-9]+)-?(SNAPSHOT){0,1}"
...
declare -a to_parts=()
declare -ar inc=(major minor build dfh snapshot)
...
version()
{
    ...
    local v=$1
    if [[ "${v}" =~ $dfh_version_regex ]]; then
        local major=${BASH_REMATCH[1]}
        local minor=${BASH_REMATCH[2]}
        local build=${BASH_REMATCH[3]}
        local dfh=true
        local dfh_build=${BASH_REMATCH[4]}
        local snapshot=false
        if [[ "SNAPSHOT" = "${BASH_REMATCH[5]}" ]]; then
            snapshot=true
        fi

        d "major=$major minor=$minor build=$build dfh=${dfh} dfh_build=$dfh_build snapshot=$snapshot"

        eval "$2=(${major} ${minor} ${build} ${dfh} ${dfh_build} ${snapshot})"
    elif [[ "${v}" =~ $version_regex ]]; then
        local major=${BASH_REMATCH[1]}
        local minor=${BASH_REMATCH[2]}
        local build=${BASH_REMATCH[3]}
        local dfh=false
        ...
        d "major=$major minor=$minor build=$build dfh=${dfh} snapshot=$snapshot"
...
if [[ -z "${to}" ]]; then
   if [[ -z "${increase}" ]]; then
    ...
    case ${increase} in
        major)
            to="$(( ${from_parts[0]} + 1 )).0.0-DFH-1"
            version "$to" to_parts
            ;;
        minor)
            to="${from_parts[0]}.$(( ${from_parts[1]} + 1 )).0-DFH-1"
            version "$to" to_parts
            ;;
        build)
            to="${from_parts[0]}.${from_parts[1]}.$(( ${from_parts[2]} + 1 ))-DFH-1"
            version "$to" to_parts
            ;;
        dfh)
            to="${from_parts[0]}.${from_parts[1]}.${from_parts[2]}-DFH-$(( ${from_parts[4]} + 1 ))"
            version "$to" to_parts
            ;;
        snapshot)
            if [[ true = ${from_parts[5]} ]]; then
                # Going from -SNAPSHOT to its release
                to="${from_parts[0]}.${from_parts[1]}.${from_parts[2]}-DFH-${from_parts[4]}"
            else
                # Going from some version to its next version and -SNAPSHOT
                to="${from_parts[0]}.${from_parts[1]}.${from_parts[2]}-DFH-$(( ${from_parts[4]} + 1 ))-SNAPSHOT"
            fi
            version "$to" to_parts
            ;;
    esac
...
if [[ ${from_parts[5]} = true && ${to_parts[5]} = true ]]; then

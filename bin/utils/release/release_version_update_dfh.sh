...
if [[ -n "$1" ]]; then
    case $1 in
        --help|-h)
            ...
            exit 1
            ;;
        major|minor|build|dfh|snapshot)
            inc="$1"
            ;;
        *)
...
${cwd}/bump_dfh.sh -f ${version} -i ${inc} ${xml_files[@]}
${cwd}/bump_dfh.sh -f ${version} -i ${inc} -s '# RELEASE_VERSION' -e '# \/RELEASE_VERSION' ${properties_files[@]}
